...
internal inline fun Path.commonVolumeLetter(): Char? {
    if (bytes.indexOf(SLASH,0) != -1) return null
...
private val Path.indexOfLastSlash: Int
    get() {
        val lastSlash = bytes.lastIndexOf(SLASH,0)
        if (lastSlash != -1) return lastSlash
        return bytes.lastIndexOf(BACKSLASH,0)
    }
...
internal inline fun Path.commonNameBytes(): ByteString {
    ...
    return when {
        lastSlash != -1 -> bytes.substring(lastSlash + 1,bytes.size)
...
internal inline fun Path.commonParent(): Path? {
    ...
    when {
        lastSlash == 2 && volumeLetter != null -> {
            ...
            return Path(bytes.substring(0,3)) // Keep the trailing '\' in C:\.
        }
        ...
        lastSlash == -1 && volumeLetter != null -> {
            ...
            return Path(bytes.substring(0, 2)) // C: is volume-relative.
        }
        lastSlash == -1 -> {
            return Path(DOT) // Parent is the current working directory.
        }
        lastSlash == 0 -> {
            return Path(bytes.substring(0, 1)) // Parent is the filesystem root '/'.
        }
        else -> {
            return Path(bytes.substring(0, lastSlash))
        }
    }
}
...
private val Path.slash: ByteString?
    get() {
        return when {
            bytes.indexOf(SLASH,0) != -1 -> SLASH
            bytes.indexOf(BACKSLASH,0) != -1 -> BACKSLASH
            else -> null
        }
    }
