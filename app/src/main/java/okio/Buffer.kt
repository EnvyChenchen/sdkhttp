...
import javax.crypto.spec.SecretKeySpec
 ...
 class Buffer : BufferedSource, BufferedSink, Cloneable, ByteChannel {
    ...
    private fun readFrom(input: InputStream, byteCount: Long, forever: Boolean) {
     ...
     fun completeSegmentByteCount(): Long = commonCompleteSegmentByteCount()
    ...
    override fun read(sink: ByteBuffer): Int {
     ...
     fun clear() = commonClear()

    @Throws(EOFException::class)
     override fun skip(byteCount: Long) = commonSkip(byteCount)

     override fun write(byteString: ByteString): Buffer = commonWrite(byteString)

     override fun write(byteString: ByteString, offset: Int, byteCount: Int) =
        commonWrite(byteString, offset, byteCount)

     override fun writeUtf8(string: String): Buffer = writeUtf8(string, 0, string.length)
    ...
    @Throws(IOException::class)
     ...
     override fun writeByte(b: Int): Buffer = commonWriteByte(b)

     override fun writeShort(s: Int): Buffer = commonWriteShort(s)

     override fun writeShortLe(s: Int) = writeShort(s.toShort().reverseBytes().toInt())

     override fun writeInt(i: Int): Buffer = commonWriteInt(i)

     override fun writeIntLe(i: Int) = writeInt(i.reverseBytes())

     override fun writeLong(v: Long): Buffer = commonWriteLong(v)

     override fun writeLongLe(v: Long) = writeLong(v.reverseBytes())

     override fun writeDecimalLong(v: Long): Buffer = commonWriteDecimalLong(v)
    ...
    override fun toString() = snapshot().toString()
     ...
     fun copy(): Buffer = commonCopy()

    /** Returns a deep copy of this buffer. */
    public override fun clone(): Buffer = copy()

     fun snapshot(): ByteString = commonSnapshot()

     fun snapshot(byteCount: Int): ByteString = commonSnapshot(byteCount)

    @JvmOverloads
     fun readUnsafe(unsafeCursor: UnsafeCursor): UnsafeCursor = commonReadUnsafe(unsafeCursor)
    ...
    fun size() = size
     ...
     class UnsafeCursor : Closeable {
        ...
        internal  var segment: Segment? = null
        @JvmField  var offset = -1L
        @JvmField  var data: ByteArray? = null
        @JvmField  var start = -1
        @JvmField  var end = -1

         fun next(): Int = commonNext()

         fun seek(offset: Long): Int = commonSeek(offset)

         fun resizeBuffer(newSize: Long): Long = commonResizeBuffer(newSize)

         fun expandBuffer(minByteCount: Int): Long = commonExpandBuffer(minByteCount)
