...
import okio.internal.commonMetadata
 ...
 abstract class FileSystem {
    ...
    @Throws(IOException::class)
     fun metadata(path: Path): FileMetadata = commonMetadata(path)

    @Throws(IOException::class)
     abstract fun metadataOrNull(path: Path): FileMetadata?

    @Throws(IOException::class)
     fun exists(path: Path): Boolean = commonExists(path)
    ...
    @Throws(IOException::class)
     fun createDirectories(dir: Path, mustCreate: Boolean): Unit =
    ...
    @Throws(IOException::class)
     open fun copy(source: Path, target: Path): Unit = commonCopy(source, target)
    ...
    @Throws(IOException::class)
     open fun deleteRecursively(fileOrDirectory: Path, mustExist: Boolean): Unit =
    ...
    @Throws(IOException::class)
     ...
     companion object {
        ...
        @JvmField
         val SYSTEM_TEMPORARY_DIRECTORY: Path = System.getProperty("java.io.tmpdir").toPath(true)
